...
keep if RAMA2D_R4 == 1 // Agriculture, livestock, hunting and related service activities related activities
collapse (mean) INGLABO, by(DPTO PER MES)


save "$data\Proc_agr_`k'_`i'.dta", replace
...
forvalues k = 1/12 {
	use "$data/Proc_agr_`i'_`k'", clear
	replace MES = `k'
	save "$data/Proc_agr_`i'_`k'", replace
}
...
forvalues k = 2020/2023 {
	...
	*use "$data\Proc_2022_1.dta", clear
	append using "$data\Proc_agr_`k'_`i'.dta"
	}
}
* special case 2020
...
* Bolívar outlier? not in agriculture sector
/*
...
eststo: reghdfe real_income fent_gr coca_gr, abs(DPTO) vce(cluster DPTO)
